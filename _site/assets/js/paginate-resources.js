...
function applyFilter(selectedYear) {
  hideAllPostsAndPagination(); // Only keep posts in the selected year
...
function displayFilterDropdown(earliestYear, currYear) {
  ...
  for (var year = currYear; year >= earliestYear; year--) {
    ...
    option.value = year;
    option.text = year;
    yearFilterMobile.appendChild(option); // Creating the a tags for desktop view
...
function changePage(curr, index) {
  ...
  changePageUtil(curr, index); // Use the number in the paginated button to figure out current page index and next page index
